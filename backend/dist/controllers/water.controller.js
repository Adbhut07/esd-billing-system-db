...
const bulkUploadWaterReadingsSchema = zod_1.z.object({
    ...
    readings: zod_1.z.array(zod_1.z.object({
        ...
        waterReading: zod_1.z.number().min(0).nullable().optional(),
    })).min(1),
});
const updateWaterReadingSchema = zod_1.z.object({
    waterReading: zod_1.z.number().min(0),
});
...
class WaterController {
    ...
    static async calculateWaterConsumption(houseId, currentReading, monthDate) {
    ...
    static async getWaterReadings(req, res) {
        try {
    ...
    static async uploadWaterReading(req, res) {
        try {
            ...
            // Calculate consumption
            const consumption = await WaterController.calculateWaterConsumption(houseId, waterReading, monthDate);
            let reading;
            if (existingReading) {
    ...
    static async bulkUploadWaterReadings(req, res) {
        try {
            // Validate request body
            ...
            for (const readingData of readings) {
                try {
                    ...
                    // Calculate consumption
                    const consumption = await WaterController.calculateWaterConsumption(house.id, waterReading, monthDate);
    ...
    static async updateWaterReading(req, res) {
        try {
            // Validate params
            ...
            const consumption = await WaterController.calculateWaterConsumption(reading.houseId, waterReading, reading.month);
            // Update reading
    ...
    static async deleteWaterReading(req, res) {
        try {
            // Validate params
            const validationResult = idParamSchema.safeParse(req.params);
            if (!validationResult.success) {
    ...
    static async recalculateWaterConsumption(req, res) {
        try {
            // Validate params
            const validationResult = idParamSchema.safeParse(req.params);
            if (!validationResult.success) {
            ...
            const consumption = await WaterController.calculateWaterConsumption(reading.houseId, reading.waterReading, reading.month);
            const updatedReading = await prisma.reading.update({
                where: { id: Number(id) },
                data: {
                    waterConsumption: consumption,
    ...
    static async recalculateAllWaterConsumption(req, res) {
        try {
