...
exports.MohallaController = void 0;
const client_1 = require("@prisma/client");
const zod_1 = require("zod");
const prisma = new client_1.PrismaClient();
// Zod validation schemas
const createMohallaSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100),
    number: zod_1.z.string().min(1).max(20),
});
const updateMohallaSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(100).optional(),
    number: zod_1.z.string().min(1).max(20).optional(),
});
const getMohallasQuerySchema = zod_1.z.object({
...
const numberParamSchema = zod_1.z.object({
    number: zod_1.z.string(),
});
class MohallaController {
    // Get all mohallas with optional search and pagination
    static async getMohallas(req, res) {
        try {
    ...
    static async getAllMohallas(req, res) {
        try {
    ...
    static async getMohallaById(req, res) {
        try {
            // Validate params
    ...
    static async getMohallaByNumber(req, res) {
        try {
            // Validate params
    ...
    static async getHousesInMohalla(req, res) {
        try {
            const { id } = req.params;
    ...
    // Create new mohalla
    static async createMohalla(req, res) {
        try {
    ...
    // Update mohalla
    static async updateMohalla(req, res) {
        try {
            // Validate params
    ...
    // Delete mohalla
    static async deleteMohalla(req, res) {
        try {
            // Validate params
            const validationResult = idParamSchema.safeParse(req.params);
            if (!validationResult.success) {
    ...
    static async getMohallaStatistics(req, res) {
        try {
            const { id } = req.params;
            if (!id) {
                return res.status(400).json({
            ...
            // Get statistics
            const [totalHouses, activeHouses, inactiveHouses, totalReadings, pendingBills, generatedBills, paidBills,] = await Promise.all([
                prisma.house.count({
    ...
    static async bulkCreateMohallas(req, res) {
        try {
...
exports.MohallaController = MohallaController;
//# sourceMappingURL=mohalla.controller.js.map