// ============================================
// src/controllers/electricity.controller.ts
// ============================================
...
const bulkUploadReadingsSchema = z.object({
  ...
  readings: z.array(
    z.object({
      ...
      importReading: z.number().min(0).nullable().optional(),
      exportReading: z.number().min(0).nullable().optional(),
      maxDemand: z.number().min(0).nullable().optional(),
    })
  ).min(1),
});

const updateElectricityReadingSchema = z.object({
  importReading: z.number().min(0).optional(),
...
export class ElectricityController {
  ...
  private static async calculateCharges(billedEnergy: number) {
  ...
  static async getElectricityReadings(req: Request, res: Response) {
    try {
  ...
  static async uploadElectricityReading(req: AuthRequest, res: Response) {
    try {
      ...
      const { consumption, billedEnergy, carryForward } =
        await ElectricityController.calculateConsumption(
          houseId,
          importReading,
          exportReading,
          monthDate
      ...
      // Calculate charges
      const { fixedCharge, electricityCharge, electricityDuty, maintenanceCharge } =
        await ElectricityController.calculateCharges(billedEnergy);
  ...
  static async bulkUploadReadings(req: AuthRequest, res: Response) {
    try {
      // Validate request body
      ...
      for (const readingData of readings) {
        try {
          ...
          if (
            readingData.importReading === null &&
  ...
  static async updateElectricityReading(req: AuthRequest, res: Response) {
    try {
      // Validate params
  ...
  static async deleteElectricityReading(req: AuthRequest, res: Response) {
    try {
      // Validate params
      const validationResult = idParamSchema.safeParse(req.params);
  ...
  static async recalculateConsumption(req: AuthRequest, res: Response) {
    try {
      // Validate params
      const validationResult = idParamSchema.safeParse(req.params);
      ...
      const { consumption, billedEnergy, carryForward } =
        await ElectricityController.calculateConsumption(
          reading.houseId,
          reading.electricityImportReading,
          reading.electricityExportReading,
          reading.month
        );
  ...
  static async initializeBaseReadings(req: AuthRequest, res: Response) {
  try {
    ...
    const results = {
      success: 0,
      failed: 0,
      errors: [] as Array<{ houseNumber: string; error: string }>,
      created: [] as Array<{ houseNumber: string; readingId: number }>,
    };
